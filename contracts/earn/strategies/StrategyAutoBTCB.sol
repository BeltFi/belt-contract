...
contract bBTCBStratAUTO is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant earnedAddress = autoAddress;
    address public constant autoFarmAddress =
    0x0895196562C7868C5Be92459FaE7f877ED450452;

    uint256 public constant poolId = 3;
    uint256 public balanceSnapshot = 0;
    ...
    address[] public earnedWantToBELTPath;
    ...
    constructor(
        ...
        autoToWantPath = [autoAddress, wbnbAddress, wantAddress];

        earnedToBELTPath = [autoAddress, wbnbAddress, BELTAddress];
        earnedWantToBELTPath = [wantAddress, wbnbAddress, BELTAddress];
    ...
    function buyBackWant(uint256 _earnedAmt) internal returns (uint256) {
        if (buyBackRate <= 0) {
            return _earnedAmt;
        }

        uint256 buyBackAmt = _earnedAmt.mul(buyBackRate).div(buyBackRateMax);
        AUTOFarm(autoFarmAddress).withdraw(poolId, buyBackAmt);
        IPancakeRouter02(uniRouterAddress).swapExactTokensForTokens(
            buyBackAmt,
            0,
            earnedWantToBELTPath,
            address(this),
            now + 600
        );
