...
contract bBETH is ERC20, ERC20Detailed, ReentrancyGuard, Ownable {
    ...
    function _supplyVenus(uint amount) internal {
        require(Venus(venus).deposit(amount) > 0, "VENUS: supply failed");
    }

    function _withdrawVenus(uint amount) internal {
        require(Venus(venus).withdraw(amount) > 0, "VENUS: withdraw failed");
    }
