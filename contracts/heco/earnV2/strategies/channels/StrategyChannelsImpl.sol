...
contract StrategyChannelsImpl is StrategyChannelsStorage {
    ...
    function _supply(uint256 _amount) internal {
        if (isWHT) {
            ...
            _unwrapHT();
            ICHT(cTokenAddress).mint{value: _amount}();
        } else {
            require(ICToken(cTokenAddress).mint(_amount) == 0, "mint Channels Err");
        }
    }

    function _removeSupply(uint256 _amount) internal {
        require(ICToken(cTokenAddress).redeemUnderlying(_amount) == 0, "redeemUnderlying Channels Err");
        if (isWHT) {
            _wrapHT();
        }
    }
    ...
    function _borrow(uint256 _amount) internal {
        require(ICToken(cTokenAddress).borrow(_amount) == 0, "borrow Channels Err");
        if (isWHT) {
            _wrapHT();
        }
    }
    ...
    function _repayBorrow(uint256 _amount) internal {
        if (isWHT) {
            // checks and reverts on error internally
            _unwrapHT();
            ICHT(cTokenAddress).repayBorrow{value: _amount}();
        } else {
            require(ICToken(cTokenAddress).repayBorrow(_amount) == 0, "repayBorrow Channels Err");
        }
    }
    ...
    function earn() external whenNotPaused {
        IChannelsDistribution(channelsDistributionAddress).claimCan(address(this));

        uint256 earnedAmt = IERC20(canAddress).balanceOf(address(this));
        earnedAmt = buyBack(earnedAmt);

        if (canAddress != wantAddress) {
    ...
    function pause() public {
        ...
        if (!isWHT) {
            IERC20(wantAddress).safeApprove(cTokenAddress, 0);
        }
    }
    ...
    function updateBalance() public view returns (uint256 sup, uint256 brw, uint256 supMin) {
        ...
        require(errCode == 0, "Channels ErrCode");
    ...
    function inCaseTokensGetStuck(
        ...
        require(_token != canAddress, "!safe");
        require(_token != wantAddress, "!safe");
        require(_token != cTokenAddress, "!safe");
    ...
    function setCANToWantPath(address[] memory newPath) public {
        require(msg.sender == govAddress, "Not authorised");
        canToWantPath = newPath;
    }
    ...
    function updateStrategy() public {
        require(ICToken(cTokenAddress).accrueInterest() == 0);
    }
